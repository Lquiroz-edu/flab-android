...
The optional `FLabAccessibilityService` receives only window-state/window-change events and the
technical package name supplied with those events. Its metadata explicitly sets:
...
It does not read nodes, text, passwords, messages, images or user input. It does not perform taps,
gestures, navigation or automated actions. No observed package data is transmitted or stored in a
history.
